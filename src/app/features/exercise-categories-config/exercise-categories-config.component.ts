...
import { ExerciseCategoryDef } from '../../core/models/category.model';
...
@Component({
  selector: 'app-exercise-categories-config',
  standalone: true,
  imports: [],
  template: `
    <div class="page">
      ...
      @if (categories().length === 0) {
        <div class="card-section">
          <div class="empty-state">
            ...
            <p>Cap categoria configurada</p>
            <button class="btn-primary" (click)="openForm()">Afegeix la primera</button>
          </div>
        </div>
      } @else {
        <div class="card-section">
          <div class="section-header">
            ...
            <h2 class="section-title">Les teves categories</h2>
            <span class="section-count">{{ categories().length }}</span>
          </div>

          @for (cat of categories(); track cat.id) {
            <div class="item-card">
              <div class="ic-bar" [style.background]="cat.color"></div>
              <span class="material-symbols-outlined cat-icon" [style.color]="cat.color">
                {{ cat.icon }}
              </span>
              <div class="ic-info">
                <span class="ic-name">{{ cat.name }}</span>
                @if (cat.muscles) {
                  <span class="ic-detail">{{ cat.muscles }}</span>
                }
              </div>
              <button class="ic-action" (click)="openForm(cat)" aria-label="Editar">
                <span class="material-symbols-outlined">edit</span>
              </button>
              <button class="ic-action ic-action--danger" (click)="deleteCategory(cat)" aria-label="Eliminar">
      ...
      <!-- FAB -->
      <button class="fab" (click)="openForm()" aria-label="Afegir categoria">
...
export class ExerciseCategoriesConfigComponent {
  ...
  openForm(category?: ExerciseCategoryDef): void {
    const ref = this.dialog.open(CategoryFormDialogComponent, {
      data: { category }, width: '360px', maxHeight: '90vh',
    });
    ref.afterClosed().subscribe(async result => {
      ...
      try {
        if (category) {
          ...
          this.feedback.success('Categoria actualitzada', 2000);
        } else {
          await this.categoryService.createCategory(result);
          this.feedback.success('Categoria creada', 2000);
        }
      } catch (err) {
  ...
  async deleteCategory(category: ExerciseCategoryDef): Promise<void> {
    if (!await this.confirmDialog.confirm(`Eliminar "${category.name}"?`, { variant: 'danger', confirmLabel: 'Eliminar' })) return;
    try {
      await this.categoryService.deleteCategory(category.id);
      this.feedback.success('Categoria eliminada', 2000);
    } catch (err) {
      if ((err as Error).message === 'IN_USE') {
        this.feedback.error('No es pot eliminar: hi ha exercicis amb aquesta categoria', 4000);
      } else {
