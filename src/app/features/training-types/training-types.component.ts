...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <header class="page-header">
        ...
        <h1>Tipus d'entrenament</h1>
      </header>

      <p class="page-hint">
        Els tipus del gimnàs que veus a la pantalla d'entrenar. Crea'n de teus —
        per exemple "Bíceps" o "Core" — a més dels de sèrie.
      </p>
      ...
      @if (types().length === 0) {
        ...
        <div class="card-section">
          <div class="section-header">
            ...
            <h2 class="section-title">Els teus tipus</h2>
            <span class="section-count">{{ types().length }}</span>
          </div>

          @for (type of types(); track type.id) {
            <div class="item-card">
              <div class="ic-bar" [style.background]="type.color"></div>
              <span class="material-symbols-outlined type-icon" [style.color]="type.color">
                {{ type.icon }}
              </span>
              <div class="ic-info">
                <span class="ic-name">{{ type.name }}</span>
                @if (type.muscles) { <span class="ic-detail">{{ type.muscles }}</span> }
              </div>
              @if (type.builtIn) {
                <span class="builtin-badge">De sèrie</span>
              }
              <button class="ic-action" (click)="openForm(type)" aria-label="Editar">
                <span class="material-symbols-outlined">edit</span>
              </button>
              <button class="ic-action ic-action--danger" (click)="deleteType(type)" aria-label="Eliminar">
      ...
      <button class="fab" (click)="openForm()" aria-label="Afegir tipus">
  ...
  styles: [`
    ...
    .page-hint {
      margin: 0 16px 4px; font-size: 12.5px; color: var(--c-text-3); line-height: 1.4;
    }
    ...
    .ic-info { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 2px; padding: 10px; }
    .ic-name { font-size: 13px; font-weight: 700; color: var(--c-text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .ic-detail { font-size: 11px; color: var(--c-text-2); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .builtin-badge {
      flex-shrink: 0; margin-right: 4px; padding: 2px 8px; border-radius: 10px;
      font-size: 10px; font-weight: 700; color: var(--c-text-3);
      background: var(--c-border-2); letter-spacing: 0.2px;
    }
    .ic-action {
...
export class TrainingTypesComponent {
  ...
  openForm(type?: TrainingType): void {
    const ref = this.dialog.open(TrainingTypeFormDialogComponent, {
      data: { type }, width: '360px', maxHeight: '90vh',
    });
  ...
  async deleteType(type: TrainingType): Promise<void> {
    // An exercise whose type disappears would become invisible in the library
    // (it groups by the live type list), so block deletion until reassigned.
    const inUse = this.exerciseService.exercises().filter(e => e.category === type.id).length;
    if (inUse > 0) {
      this.feedback.error(
        `Aquest tipus té ${inUse} exercici${inUse === 1 ? '' : 's'}. Reassigna'ls o esborra'ls primer.`, 4000,
      );
      return;
    }
    const extra = type.builtIn
      ? ' És un tipus de sèrie; els entrenaments antics d\'aquest tipus es mantindran.'
      : ' Els entrenaments antics d\'aquest tipus es mantindran.';
